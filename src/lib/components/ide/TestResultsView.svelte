<script lang="ts">
	...
	import { CheckCircle2, XCircle, ArrowRight, ShieldCheck, AlertCircle } from '@lucide/svelte';

	let { results = null, onNextModule = () => {} } = $props<{
		...
		{#if results}
			...
			{#if results.allPassed}
				<div class="mb-4 border border-border bg-secondary p-4">
					...
					<button
						type="button"
						class="mt-3 flex w-full items-center justify-center gap-2 bg-primary px-3 py-2 font-mono text-xs font-bold tracking-wider text-primary-foreground uppercase transition-colors hover:opacity-85"
