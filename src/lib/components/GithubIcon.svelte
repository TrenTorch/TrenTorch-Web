<script lang="ts">
	// Not a lucide icon: lucide dropped brand/logo marks from its icon set,
	// so this is GitHub's own Octicon mark (MIT licensed), sized to match
	// the rest of this project's lucide icons.
	let { class: className = '' }: { class?: string } = $props();
</script>

<svg viewBox="0 0 16 16" class={className} fill="currentColor" aria-hidden="true">
	<path
