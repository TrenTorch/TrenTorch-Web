...
import { markModuleCompleted } from './storage';
...
class PyodideService {
			...
			this.worker.onmessage = (e: MessageEvent) => {
				...
				if (type === 'test_result') {
					...
					if (req) {
						...
						if (subResult.allPassed) {
							markModuleCompleted(subResult.moduleId);
						}
	...
	public async runTests(
		...
		this.init();
		this.isRunning.set(true);
		this.consoleOutput.set(`Running test suite for [${moduleId}]...\n`);
