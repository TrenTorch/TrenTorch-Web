...
- `fit_gmm` returns a dict with `"weights"`, `"means"`, `"variances"` (final, post-loop), `"responsibilities"` (from the *last* E-step), and `"log_likelihood_history"` (array, one entry per iteration, recorded after that iteration's M-step).
