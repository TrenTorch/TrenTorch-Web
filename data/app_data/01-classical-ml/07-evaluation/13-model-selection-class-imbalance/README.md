...
`stratified_k_fold_split` runs `01-splitting-and-resampling`'s own splitting idea _separately per class_ first (shuffle each class's own indices, split into `k` pieces with `np.array_split`), then combines fold `i` across every class to build that fold's validation set.
